...
interface SignUpUseCase {
    suspend operator fun invoke(login: String, password: String): SignUpResponse//Response<SignUpResponse>//fixme signIn response token
}

class SignUpUseCaseImpl @Inject constructor(private val loginRepository: LoginRepository) :
    SignUpUseCase {

    override suspend operator fun invoke(login: String, password: String): SignUpResponse {//Response<SignUpResponse> {
       /*val jsonObject = JsonObject()
        ...
        val signUpResponse = loginRepository.signUp(userInfo)
